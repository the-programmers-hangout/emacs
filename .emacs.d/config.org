...
** Package Archives
   ...
   All of the third-party packages we’re going to reference are available through MELPA (<https://melpa.org/>). This is your one stop shop for finding and installing Emacs addons. Run `M-x list-packages` to bring up an interactive searchable list. When the point is over a package line, hit `i` to mark the package for installation, then `x` to install all marked packages. If packages have updates, mark them all with `U`.
...
* Code completion
  ...
  Company is the primary package that is used for code completion, it follows a frontend/backend system. The package =company= is the frontend, it will query a certain backend based on what code you are editing.
